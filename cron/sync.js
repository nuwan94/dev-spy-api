...
cron.schedule(cronInterval, async function () {
  ...
  if (stackoverflow.data) {
    await StackOverflow.findOneAndUpdate(
      { account_id: stackoverflow.data.items[0].account_id },
      ...
      (err) => {
        if (err) {
          ...
          console.log(
            "StackOverflow Success : Data Saved Successfully",
            stackoverflow.data
          );
        }
      }
...
cron.schedule(cronInterval, async function () {
  ...
  await axios.get(process.env.MEDIUM_URI).then((response) => {
    ...
    followers_count = $('button:contains("Followers")')
      .first()
      .text()
      .split(" ")[0];
  ...
  await axios.get(process.env.MEDIUM_JSNO_FEED_URI).then((res) => {
...
cron.schedule(cronInterval, async function () {
  ...
  await axios.get(process.env.QWIKLABS_URI).then((response) => {
    const $ = cheerio.load(response.data);
    const stats = $(".public-profile__hero__details")
      .text()
      .replace(/\n/g, " ")
      .trim();
    labs = stats.split(" ")[0];
  ...
  if (qwiklabsData) {
    await Qwiklabs.findOneAndUpdate(
      { username: qwiklabsData.username },
      qwiklabsData,
      {
        upsert: true,
      },
      (err) => {
        if (err) {
          ...
          console.log(
            "Qwiklabs Success : Data Saved Successfully",
            qwiklabsData
          );
        }
      }
