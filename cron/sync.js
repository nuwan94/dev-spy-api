...
cron.schedule(cronInterval, async function() {
  ...
  if (mediumData) {
    await Medium.findOneAndUpdate(
        {username : mediumData.username}, mediumData, {
          upsert : true,
        },
        (err) => {
          if (err) {
...
cron.schedule(cronInterval, async function() {
  ...
  await axios.get(process.env.QWIKLABS_URI).then((response) => {
    const $ = cheerio.load(response.data);
    const stats =
        $('.public-profile__hero__details').text().replace(/\n/g, ' ').trim();
    labs = stats.split(" ")[0];
  ...
  if (qwiklabsData) {
    await Qwiklabs.findOneAndUpdate(
        {username : qwiklabsData.username}, qwiklabsData, {
          upsert : true,
        },
        (err) => {
          if (err) {
